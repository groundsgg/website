...
var worker_default = {
  async fetch(request, env) {
    ...
    const final = res.status === 404 ? await env.ASSETS.fetch(new Request(new URL("/404.html", request.url))) : res;
    return withSecurityHeaders(final, res.status === 404 ? 404 : final.status);
  }
};
function withSecurityHeaders(res, status = 200) {
...
// .wrangler/tmp/bundle-fU8PKf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fU8PKf/middleware-loader.entry.ts
